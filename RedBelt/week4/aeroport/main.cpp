...
public:
    ...
    using Items = std::array<Item ,(int)TAirport::Last_>;
    ...
    Items GetItems() const{
        Items ret;
        for (int i = 0; i < (int)TAirport::Last_;++i)
            ret[i]={static_cast<TAirport>(i),data[i]};

        std::sort(ret.begin(),ret.end(),[](const Item& lhs, const Item& rhs){
...
private:
    std::array<size_t, (int)TAirport::Last_> data;
};
